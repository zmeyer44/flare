...
export default function EditVideoPage({ event }: EditVideoPageProps) {
  ...
  const [videoData, setVideoData] = useState<{
    ...
    url: getTagValues("url", event.tags),
    title: getTagValues("title", event.tags),
    summary: event.content,
    thumbnail: getTagValues("thumb", event.tags),
    fileType: getTagValues("m", event.tags),
    fileHash: getTagValues("x", event.tags),
    fileSize: isNaN(parseInt(getTagValues("size", event.tags) ?? ""))
      ? undefined
      : parseInt(getTagValues("size", event.tags) ?? ""),
    duration: isNaN(parseInt(getTagValues("duration", event.tags) ?? ""))
      ? undefined
      : parseInt(getTagValues("duration", event.tags) ?? ""),
    hashtags: getTagsValues("t", event.tags).join(","),
    contentWarning: getTagValues("content-warning", event.tags),
  });
  ...
  async function handleSubmit() {
    ...
    const d = getTagValues("d", event.tags);
    if (!videoData?.url || !videoData?.title || !d) return;
    setLoading(true);
    try {
      const tags: string[][] = [
        ["d", d],
      ...
      if (newEvent) {
        ...
        toast.success("Updated Video published!");
        const encodedEvent = newEvent.encode();
