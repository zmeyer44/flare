...
const useUploadVideo = (props?: UploadProps) => {
  ...
  const uploadVideo = async (file: File, folderName?: string) => {
    if (!file) return;
    try {
  ...
  const onVideoChange = (e: React.FormEvent<HTMLInputElement>) => {
    const file = e.currentTarget.files?.[0];
    if (!file) return;
    setStatus("uploading");
    uploadVideo(file, folderName);
